...
function getStyle(node, prop) {
  if (typeof node === "string") node = document.querySelector(node);
  if (typeof prop !== "string")
    throw new Error("getStyle 함수의 두 번째 인수는 문자 타입 이어야 한다.");
  return console.log(node.style[prop] getComputedStyle(node)[prop]);
}

getStyle(".first", "fontSize"); //32px

// ------ //
// set, get Style 나뉜 게 불편! 한번에 만들면?
...
function css(node, prop, value) {
  ...
  return value ? setStyle(node, prop, value) : getStyle(node, prop);
  //setStyle 은 어차피 Undefined를 제출함! 그래서 setStyle에서 언디파인드가 튀어나와도 괜찮음!
}
